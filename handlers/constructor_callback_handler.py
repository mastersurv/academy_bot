...
async def constructor_callback_handler(call: CallbackQuery, state: FSMContext):
    ...
    if callback == "menu":
    ...
    elif callback.startswith("course_settings"):
        course_id = int(callback.split("_")[2])
        ...
        await bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup().add(
                InlineKeyboardButton(
                    text="Да, я уверен",
                    callback_data=f"confirm_deletion_{course_id}"
                ),
                InlineKeyboardButton(
        ...
        message_text = f"{promo_text}" \
                       "Для выдачи пользователю доступа к курсу отправьте ему необходимый промокод. " \
                       "Он должен перейти в меню и выбрать 'Получить курс'. " \
                       "После этого ему будет выдан доступ.\n\n" \
                       "Если вы хотите создать промокод, который можно использовать только n-ное  кол-во раз, то перейдите по кнопке СОЗДАТЬ N-РАЗОВЫЙ ПРОМОКОД\n\n" \
                       "Если вы хотите дать доступ к курсу только определенной чат-группе, перейдите по кнопке - ПРИВЯЗАТЬ ПРОМОКОД К ЧАТ-ГРУППЕ" \
 \
        # Создаем Inline Keyboard Markup с кнопкой "Назад"
        keyboard = InlineKeyboardMarkup()
        keyboard.add(
            InlineKeyboardButton(text="Создать n-разовый промокод", callback_data=f"generate_n_promo_{course_id}"))
        keyboard.add(InlineKeyboardButton(text="Привязать прокомод к чат-группе",
                                          callback_data=f"generate_group_promo_{course_id}"))
        keyboard.add(InlineKeyboardButton(text="Назад", callback_data="courses_promocodes"))

        # Отправляем сообщение пользователю
        await bot.edit_message_text(chat_id=chat, text=message_text, message_id=m_id, parse_mode='html',
                                    reply_markup=keyboard)
    ...
    # owners section
    elif callback == "button_post":
        await MenuStates.button_post.set()

        await bot.send_message(
            chat_id=tg_id,
            text="Отправьте мне пост, к которому хотите приделать кнопку.\n"
                 "Я отправлю его в канал с кнопкой - ссылкой на бота"
        )
