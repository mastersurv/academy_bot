...
async def statistics_to_course(tg_id: int, course_id: int):
    ...
    for ci in courses_ids:
        ...
        right_answer = await db.get_positive_count(tg_id=tg_id, course_id=ci)
        wrong_answer = await db.get_negative_count(tg_id=tg_id, course_id=ci)
        start_time, end_time = await db.get_completion(tg_id=tg_id, course_id=ci)
