...
async def handle_module_button(call: types.CallbackQuery):
	...
	lesson_keyboard = await generate_lessons_keyboard(int(course_id), int(module_id))
...
async def handle_lesson_button(call: types.CallbackQuery):
	await call.answer()
	lesson_id = call.data.split('_')[3]
...
# Кнопка назад (Выбор модуля)
@dp.callback_query_handler(text_startswith='back_modules')
async def back_courses(call: types.CallbackQuery):
	await call.answer()
	course_id = call.data.split('_')[2]
	keyboard = await generate_modules_keyboard(int(course_id))
	await call.message.edit_text(f"Описание курса {course_id}", reply_markup=keyboard)
