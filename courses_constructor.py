...
from states_handlers.button_post_handler import button_post_handler

from commands_handlers.set_group_handler import set_group_handler
from commands_handlers.admin_handler import admin_handler
...
class MyBot:
	...
	def register_handlers(self):
		...
		self.dp.register_message_handler(
			callback=get_promocode_handler, content_types=["text"], state=MenuStates.promocode
		)
		self.dp.register_message_handler(
			callback=get_promo_usage_handler, content_types=["text"], state=MenuStates.n_promo
		)
		self.dp.register_message_handler(
			callback=button_post_handler, content_types=ContentTypes.ANY, state=MenuStates.button_post
		)

		self.dp.register_message_handler(
			callback=edit_course_name, state=SettingsStates.course_name, content_types=["text"]
		)
		self.dp.register_message_handler(
			callback=edit_course_description, state=SettingsStates.course_description, content_types=["text"]
		)
		self.dp.register_message_handler(
			callback=edit_course_image, state=SettingsStates.course_image, content_types=["photo"]
		)
		self.dp.register_message_handler(
			callback=edit_final_message, state=SettingsStates.final_message, content_types=ContentTypes.ANY
		)

		self.dp.register_message_handler(
			callback=edit_module_name, state=SettingsStates.module_name, content_types=["text"]
		)
		self.dp.register_message_handler(
			callback=edit_module_description, state=SettingsStates.module_description, content_types=["text"]
		)
		self.dp.register_message_handler(
			callback=edit_module_image, state=SettingsStates.module_image, content_types=["photo"]
		)

		self.dp.register_message_handler(
			callback=edit_lesson_name, state=SettingsStates.lesson_name, content_types=["text"]
		)
		self.dp.register_message_handler(
			callback=edit_lesson_description, state=SettingsStates.lesson_description, content_types=["text"]
		)
		self.dp.register_message_handler(
			callback=edit_lesson_material, state=SettingsStates.lesson_material, content_types=ContentTypes.ANY
		)
		self.dp.register_message_handler(
			callback=test_question_handler, state=SettingsStates.test_question, content_types=["text"]
		)
		self.dp.register_message_handler(
			callback=test_keyboard_handler, state=SettingsStates.test_keyboard, content_types=["text"]
		)
